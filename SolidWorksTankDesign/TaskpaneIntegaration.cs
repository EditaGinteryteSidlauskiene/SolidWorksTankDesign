...
{
    /*To install this addin, make sure that TaskpaneIntegration calss is public.
     There are two options to install:
    1. Install the SolidWorksAddinInstaller from github anglesix (https://github.com/angelsix/solidworks-api/tree/develop/Tools/Addin%20Installer)
    2. Run Administrator Command Prompt as admintrator. C:\Windows\Microsoft.NET\Framework64\v4.0.30319>RegAsm.exe <here should be the path of dll file, which is in bin-Debug (e.g. "C:\Users\Edita\Desktop\SolidWorksTankDesign\SolidWorksTankDesign\bin\Debug\SolidWorksTankDesign.dll")>
    Hit Enter.Run SolidWorks to check if UI is displayed.
     */
    /// <summary>
    ...
    public class TaskpaneIntegration : ISwAddin
    {
        ...
        /* Variable of type of TaskpaneHostUI.cs [Design] control, because we are going to create an instance of it. The UI control that is going to be inside the SolidWorks taskpane view.*/
    private TaskpaneHostUI mTaskpaneHostUI;
      ...
      The unique Id to the taskpane used for registration in COM*/
        public const string SWTASKPANE_PROGID = "Edita.SolidWorksTankDesign.Taskpane";
        ...
        private void LoadUI()
        {
           ...
           var imagePath = Path.Combine(Path.GetDirectoryName(typeof(TaskpaneIntegration).Assembly.CodeBase).Replace(@"file:\", string.Empty), "logo-small.bmp");
            ...
            mTaskpaneHostUI = (TaskpaneHostUI)mTaskpaneView.AddControl(TaskpaneIntegration.SWTASKPANE_PROGID, string.Empty);

            //// Passing SW application reference to TaskpaneHostUI class.
            //mTaskpaneHostUI.AssignSWApp(mSolidWorksApplication);
        }
        ...
        private static void ComRegister(Type t)
        {
            ...
            using (var rk = Microsoft.Win32.Registry.LocalMachine.CreateSubKey(keyPath))
            {
                ...
                rk.SetValue("Title", "SolidWorks Tank Design");
